...
sim <- NegBinCopula$new(mean_formula="~ bs(pseudotime, df=6, degree=3)", dispersion_formula="~ bs(pseudotime, df=5, degree = 1)", copula_formula="~ -1 + cell_type")
sim$fit(sce, max_epochs=40, batch_size=512, lr=.5)
```
...
plot_df |>
    filter(gene %in% rownames(sce)[1:8]) |>
